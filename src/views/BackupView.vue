...
import BackupList from "@components/views/backup/BackupList.vue";
import CreateBackupModal from "@components/views/backup/CreateBackupModal.vue";

const serverStore = useServerStore();
const backupStore = useBackupStore();

const showCreateModal = ref(false);
const { loading: actionLoading } = useLoading(false);
...
onMounted(async () => {
  ...
  if (selectedServerId.value) {
    await backupStore.refreshBackups(selectedServerId.value);
  }
});

watch(selectedServerId, async (sid) => {
  if (sid) {
    await backupStore.refreshBackups(sid);
  }
});
